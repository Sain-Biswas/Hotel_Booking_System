...
if($conn->connect_error){
    ...
    $stmt = $conn->prepare("update booking_info set title = ? , first_name = ? , last_name = ? , from_date = ? , to_date = ? , email = ? , mobile = ? , 4d_iden = ? where booking_id = ?");
    $stmt->bind_param("ssssssisi",$title,$fname,$lname,$cidate,$coudate,$email,$mob,$perid,$numr);
    $stmt->execute();
    $sss = "Updated Successfully...";
    $conpic = "/pictures/reg-conf.png";
    $ppp =  " <br> Booking id : ".$numr ;
    $stmt->close();
    $conn->close();
}
?>
